...
public class FOLD 
{
    ...
    private double getClauseBits(Clause x_cl)
    {
        ...
        for(Predicate p:x_cl.Body)
        {
            ...
            dTotal = dTotal  + l + variablization;
        }        
    ...
    private Clause extend_clause_loop(Clause x_clause,ArrayList<Predicate> x_PosExamples,ArrayList<Predicate> x_NegExamples)
    {
        ...
        while(!bExit)
        {
            ...
            Pair<Clause,Double> pair_neg = best_next_clause(x_clause,x_NegExamples,x_PosExamples);
            // Learning from Positive literals failed for some reason including lengthy hypothesis or lack of gainful litera
            if((pair != null && pair_neg != null && pair_neg.getValue() > 2 * pair.getValue()))
            {
                System.out.printf("Happened--> %s\r\n", pair_neg.getKey().toString());
            }
            if(pair != null && !(pair_neg != null && pair_neg.getValue() > 2 * pair.getValue())) 
            {
                 c_new = pair.getKey();
            ...
            else
            {
                ...
                x_NegExamples = covered_examples(x_clause, x_NegExamples);
